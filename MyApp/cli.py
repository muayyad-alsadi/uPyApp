...
def help(command=None):
    ...
    if command:
        help_one(command, complete=True)
        return
...
def help_one(name, func=None, complete=False):
    ...
    args_help=inspect.formatargspec(args, varargs, varkw, defaults, formatvalue=lambda s: "=["+str(s)+"]")[1:-1].replace(', ', ' ')
...
def main(app_config=None):
    ...
    for option in options:
        if '=' not in option: ls.append(option)
        else:
